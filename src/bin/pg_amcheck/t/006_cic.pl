...
$node->safe_psql('postgres', q(CREATE TABLE tbl(i int primary key,
								...
								ia int4[], p point)));
# uncomment to force no-HOT $node->safe_psql('postgres', q(CREATE INDEX CONCURRENTLY idx ON tbl(i, updated_at);));
# create sequence
...
$node->pgbench(
	'--no-vacuum --client=30 --jobs=4 --exit-on-abort --transactions=10000',
	0,
	[qr{actually processed}],
	[qr{^$}],
