...
typedef struct BTBuildState
{
	...
	/*
	 * spool2 is needed only when the index is a unique index and built non-concurrently.
					...
					IndexInfo *indexInfo)
{
	 ...
	 * It is required because of periodic reset of snapshot.
	 */
	...
	if (fail_on_alive_duplicate)
	{
		...
		while ((itup = tuplesort_getindextuple(btspool->sortstate, true)) != NULL)
		{
			...
			if (prev != NULL) /* if is not the first tuple */
			{
				...
				/* if this tuples equal to previous one? */
