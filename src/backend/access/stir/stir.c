...
StirFillMetapage(Relation index, Page metaPage, bool skipInserts)
{
	...
	metadata->magickNumber = STIR_MAGIC_NUMBER;
		  ...
		  struct IndexInfo *indexInfo)
{
	...
	/* Create new tuple with a heap pointer */
									 ...
									 void *callback_state)
{
	...
	if (!info->validate_index)
	{
		...
		ereport(WARNING, (errcode(ERRCODE_FEATURE_NOT_SUPPORTED),
				errmsg("\"%s\" is not implemented, seems like this index need to be dropped", __func__)));
		return NULL;
	}
