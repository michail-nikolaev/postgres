				  ...
				  RelOptInfo *rel)
{
	...
	if (hasindex)
	{
		...
		/*
		 * The window replay can get into: the index list is in hand, and no
		 * index is open yet.  The table's name is passed so that a test can
		 * park a backend here for one specific relation.
		 */
		INJECTION_POINT("get-relation-info-before-index-open",
						RelationGetRelationName(relation));
		...
		foreach(l, indexoidlist)
		{
			 ...
			 * that holds only the table's lock and has already read the index
			 * list.  Simply ignore an index that is no longer there: it is
			 * one we must not plan with in any case.
			 */
