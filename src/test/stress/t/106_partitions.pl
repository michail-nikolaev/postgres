...
#
# Without 90eae926abb, "Fix ON CONFLICT with REINDEX CONCURRENTLY and
# partitions", which lets an index being rebuilt stand in as an arbiter
# for the partition, this fails one run in six at stress_concurrently=4
# with a duplicate key error against a partition's transient index.  It
# needs partition_upsert_contend: partition_upsert only ever meets rows
# that already exist, so it never reaches speculative insertion.
#
