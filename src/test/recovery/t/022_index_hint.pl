...
$node_primary->safe_psql('postgres',
    'INSERT INTO test_index_hint VALUES (generate_series(1, 20), 0)');
...
# Make sure sender_propagates_feedback_to_primary is set on standby
sleep(3);
...
my $result =
    $node_primary->safe_psql('postgres', "SELECT count(*) FROM bt_page_items('test_index', 1) WHERE dead = true");
is($result, qq(0), 'no index hints are set on primary yet');
...
$result =
    $node_standby->safe_psql('postgres', "SELECT count(*) FROM bt_page_items('test_index', 1) WHERE dead = true");
is($result, qq(0), 'no index hints are set on standby yet');
...
is($result, qq(0), 'all items are normal in heap');

$result =
    $node_primary->safe_psql('postgres', "SELECT count(*) FROM bt_page_items('test_index', 1) WHERE dead = true");
is($result, qq(10), 'index hints are set on primary already');
