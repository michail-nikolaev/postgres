...
-- No scenario below (until the cursor ones) is expected to leave xmin pinned
-- when a build phase starts or ends; enforce that strictly.
SELECT injection_points_attach('index_build_phase_entered_pinned', 'error');
SELECT injection_points_attach('index_build_allow_xmin_advance_pinned', 'error');
-- Nothing in the standard battery executes SQL in a phase that holds no
-- snapshot, so the build must never have to supply one.
SELECT injection_points_attach('index_build_phase_needs_snapshot', 'error');
...
-- An opclass support function may itself execute SQL, and the phases that
-- call it hold no snapshot: the region has to supply one on demand.  Watch
-- that happen instead of treating it as a failure.
SELECT injection_points_detach('index_build_phase_needs_snapshot');
SELECT injection_points_attach('index_build_phase_needs_snapshot', 'notice');

CREATE FUNCTION cic_reset_snap.cmp_sql(int, int) RETURNS int IMMUTABLE
LANGUAGE plpgsql AS $$
BEGIN
    EXECUTE 'SELECT 1';
...
CREATE OPERATOR CLASS cic_reset_snap.int4_sql_ops FOR TYPE int4 USING btree AS
    OPERATOR 1 <, OPERATOR 2 <=, OPERATOR 3 =, OPERATOR 4 >=, OPERATOR 5 >,
    FUNCTION 1 cic_reset_snap.cmp_sql(int, int);

CREATE TABLE cic_reset_snap.opclass_tbl(i int);
ALTER TABLE cic_reset_snap.opclass_tbl SET (parallel_workers=0);
INSERT INTO cic_reset_snap.opclass_tbl SELECT g FROM generate_series(1, 200) g;
CREATE INDEX CONCURRENTLY opclass_idx
    ON cic_reset_snap.opclass_tbl(i cic_reset_snap.int4_sql_ops);
REINDEX INDEX CONCURRENTLY cic_reset_snap.opclass_idx;
DROP INDEX CONCURRENTLY cic_reset_snap.opclass_idx;

-- The same in a parallel build.  The leader could fall back on the portal
-- snapshot of the CREATE INDEX statement, at the price of pinning the horizon
-- for the rest of the build, but a worker has no portal at all: the region is
-- the only thing that can hand it one.  Which participant reaches the phase
-- first varies, so stop reporting the supplied snapshots.
SELECT injection_points_detach('index_build_phase_needs_snapshot');
ALTER TABLE cic_reset_snap.opclass_tbl SET (parallel_workers=2);
CREATE INDEX CONCURRENTLY opclass_idx
    ON cic_reset_snap.opclass_tbl(i cic_reset_snap.int4_sql_ops);
DROP TABLE cic_reset_snap.opclass_tbl;
...
-- The leaked registration keeps xmin pinned, so the pinned injection points
-- fire; observe the phase-boundary one and mute the others.
SELECT injection_points_detach('index_build_allow_xmin_advance_pinned');
SELECT injection_points_attach('index_build_allow_xmin_advance_pinned', 'notice');
SELECT injection_points_detach('index_build_phase_entered_pinned');
...
-- leaked registration of the scan snapshot is seen at the end of the scan.
...
-- at the next snapshot reset.  Detach the pinned point: the number of resets
-- after the cursor is opened depends on the page layout.
SELECT injection_points_detach('index_build_allow_xmin_advance_pinned');
