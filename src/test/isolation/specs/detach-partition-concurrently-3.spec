...
setup
{
  ...
  CREATE TABLE d3_idp (a int, b bigint GENERATED BY DEFAULT AS IDENTITY)
    PARTITION BY LIST(a);
  CREATE TABLE d3_idp1 PARTITION OF d3_idp FOR VALUES IN (1);
  CREATE TABLE d3_idp2 PARTITION OF d3_idp FOR VALUES IN (2) PARTITION BY LIST(a);
  CREATE TABLE d3_idp2_leaf (b bigint NOT NULL, a int);
  ALTER TABLE d3_idp2 ATTACH PARTITION d3_idp2_leaf FOR VALUES IN (2);
}
...
step s2detachid		{ ALTER TABLE d3_idp DETACH PARTITION d3_idp1 CONCURRENTLY; }
step s2detachid2	{ ALTER TABLE d3_idp DETACH PARTITION d3_idp2 CONCURRENTLY; }
...
# partition pending detach has none.  The climb to the root must also get past
# an intermediate partition pending detach, and map the column by name: the
